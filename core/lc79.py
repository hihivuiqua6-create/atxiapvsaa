...
bp = Blueprint('gamelc79', __name__)
...
# Nguồn dữ liệu (theo lcmd5.js / lchu.js)
API_URLS = {
...
def parse_lines(data):
    """Parse danh sách phiên từ API (logic parseLines trong lchu.js / lcmd5.js)"""
...
def start_fetching():
    ...
    tx_history = load('lc79-tx')
    md5_history = load('lc79-md5')
    ...
    threading.Thread(target=poll_loop, args=('tx', latest_result, tx_history, 'lc79-tx'), daemon=True).start()
    threading.Thread(target=poll_loop, args=('md5', latest_result_md5, md5_history, 'lc79-md5'), daemon=True).start()


@bp.route("/lc79/api/tx", methods=['GET'])
def get_tx():
    """Endpoint trả về kết quả TX mới nhất"""
    return jsonify(latest_payload(latest_result, build_cau(tx_history)))

@bp.route("/lc79/api/md5", methods=['GET'])
def get_md5():
    """Endpoint trả về kết quả TX MD5 mới nhất"""
    return jsonify(latest_payload(latest_result_md5, build_cau(md5_history)))

@bp.route("/lc79/api/tx/history", methods=['GET'])
def get_tx_history():
    """Endpoint trả về lịch sử TX"""
    return jsonify({"history": decorate_history(tx_history), "total": len(tx_history)})

@bp.route("/lc79/api/tx/history/<int:n>", methods=['GET'])
