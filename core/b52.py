...
API_BASE_URL = 'https://jakpotgwab.geightdors.net/glms/v1/notify/taixiu?platform_id=b5&gid=vgmn_{}'
...
def update_result(store, history, game_data, is_md5=False):
    ...
    save('b52-md5' if is_md5 else 'b52-tx', history)
...
def fetch_game_data(gid, store, history, is_md5):
    """Lấy dữ liệu từ API nguồn theo logic b52.js"""
    global sid_for_tx, last_sid100, last_sid101
...
def start_fetching():
    ...
    history100 = load('b52-tx')
    history101 = load('b52-md5')
    sicbo_history = load('b52-sicbo')
...
def to_sicbo_entry(item):
    """Chuyển 1 record sicbo thành entry format TX (b52)"""
...
@bp.route("/b52/api/sicbo/history/<int:n>", methods=['GET'])
