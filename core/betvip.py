...
bp = Blueprint('gamebetvip', __name__)
...
# Nguồn dữ liệu (theo bethu.js / betmd5.js)
API_URLS = {
    'tx': 'https://wtx.macminim6.online/v1/tx/sessions',
    'md5': 'https://wtxmd52.macminim6.online/v1/txmd5/sessions'
}
...
def parse_lines(data):
    """Parse danh sách phiên từ API (logic parseLines trong bethu.js)"""
    items = data.get("list") or []
    items = sorted(items, key=lambda x: x.get("id") or 0)
    return [{
        "session": item.get("id"),
        "dice": item.get("dices", []),
        "total": item.get("point"),
        "result": item.get("resultTruyenThong")
    } for item in items if item.get("dices")]


def to_entry(record):
    """Chuyển 1 record phiên thành entry format 789"""
    dice = record["dice"]
    total = record["total"] or sum(dice)
    return {
...
def fetch_and_process(gid, store, history, file_key):
    """Lấy dữ liệu và xử lý phiên mới (logic fetchAndProcessHistory trong JS)"""
    global last_session
    try:
...
def start_fetching():
    ...
    tx_history = load('betvip-tx')
    md5_history = load('betvip-md5')
    ...
    threading.Thread(target=poll_loop, args=('tx', latest_result, tx_history, 'betvip-tx'), daemon=True).start()
    threading.Thread(target=poll_loop, args=('md5', latest_result_md5, md5_history, 'betvip-md5'), daemon=True).start()


@bp.route("/betvip/api/tx", methods=['GET'])
def get_tx():
    """Endpoint trả về kết quả TX mới nhất"""
    return jsonify(latest_payload(latest_result, build_cau(tx_history)))

@bp.route("/betvip/api/md5", methods=['GET'])
def get_md5():
    """Endpoint trả về kết quả TX MD5 mới nhất"""
    return jsonify(latest_payload(latest_result_md5, build_cau(md5_history)))

@bp.route("/betvip/api/tx/history", methods=['GET'])
def get_tx_history():
    """Endpoint trả về lịch sử TX"""
    return jsonify({"history": decorate_history(tx_history), "total": len(tx_history)})

@bp.route("/betvip/api/tx/history/<int:n>", methods=['GET'])
