...
<style>
:root{--bg:#07090f;--panel:#0e1420;--line:#1d2636;--txt:#e6ecf5;--dim:#8595ad;--acc:#34d399}
*{box-sizing:border-box}
body{font-family:'DM Sans',system-ui,sans-serif;background:
 radial-gradient(900px 500px at 10% -10%,#12203a 0,transparent 60%),
 radial-gradient(800px 500px at 100% 0,#1a1030 0,transparent 55%),var(--bg);
 color:var(--txt);margin:0;padding:32px 24px 64px;min-height:100vh}
header{max-width:1200px;margin:0 auto 24px}
h1{font-family:'Space Grotesk',sans-serif;font-size:28px;letter-spacing:-.02em;margin:0 0 4px}
.sub{color:var(--dim);font-size:13px}
.stats{max-width:1200px;margin:0 auto 28px;display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:12px}
.stat{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:14px 16px}
.stat b{font-family:'Space Grotesk',sans-serif;display:block;font-size:24px;line-height:1.1}
.stat span{font-size:12px;color:var(--dim)}
.wrap{max-width:1200px;margin:0 auto;display:grid;grid-template-columns:repeat(auto-fill,minmax(330px,1fr));gap:16px}
.card{background:linear-gradient(180deg,#101828,#0c1220);border:1px solid var(--line);border-radius:16px;padding:18px;transition:.18s}
.card:hover{border-color:#2c3b53;transform:translateY(-2px)}
.card h2{font-family:'Space Grotesk',sans-serif;margin:0 0 4px;font-size:17px;display:flex;justify-content:space-between;align-items:center;gap:8px;text-transform:capitalize}
.small{font-size:11.5px;color:var(--dim);margin:0 0 12px}
...
a:hover{color:#fff}
.badge{padding:4px 10px;border-radius:999px;font-size:11px;font-weight:600;white-space:nowrap;border:1px solid transparent}
.ok{background:rgba(52,211,153,.12);color:#34d399;border-color:rgba(52,211,153,.35)}
.cham{background:rgba(251,191,36,.12);color:#fbbf24;border-color:rgba(251,191,36,.35)}
.ngung,.error{background:rgba(248,113,113,.12);color:#f87171;border-color:rgba(248,113,113,.35)}
.idle,.unknown{background:rgba(148,163,184,.12);color:#94a3b8;border-color:rgba(148,163,184,.3)}
.dot{display:inline-block;width:6px;height:6px;border-radius:50%;background:currentColor;margin-right:6px;vertical-align:middle}
</style></head>
<body>
<header>
<h1>Core API Dashboard</h1>
<p class="sub">1 cổng :{{ port }} · HTTP + WebSocket · tự làm mới mỗi 8 giây</p>
</header>
<div class="stats">
<div class="stat"><b style="color:#34d399">{{ stats.ok }}</b><span>Đang hoạt động</span></div>
<div class="stat"><b style="color:#fbbf24">{{ stats.cham + stats.ngung }}</b><span>Chậm / ngừng</span></div>
<div class="stat"><b style="color:#f87171">{{ stats.error }}</b><span>Bị lỗi</span></div>
<div class="stat"><b style="color:#94a3b8">{{ stats.idle }}</b><span>Chưa có dữ liệu</span></div>
</div>
<div class="wrap">
{% for r in rows %}
<div class="card">
<h2>{{ r.name }}
{% if r.status == 'ok' %}<span class="badge ok"><i class="dot"></i>Hoạt động</span>
{% elif r.status == 'cham' %}<span class="badge cham"><i class="dot"></i>Chậm {{ r.age }}s</span>
{% elif r.status == 'ngung' %}<span class="badge ngung"><i class="dot"></i>Ngừng {{ r.age }}s</span>
{% elif r.status == 'idle' %}<span class="badge idle"><i class="dot"></i>Chưa có dữ liệu</span>
{% elif r.status == 'error' %}<span class="badge error"><i class="dot"></i>Lỗi</span>
{% else %}<span class="badge unknown"><i class="dot"></i>{{ r.status }}</span>{% endif %}
</h2>
<p class="small">Cập nhật: {{ r.last_update or '—' }} · kiểm tra {{ r.checked or '—' }}</p>
<table>
